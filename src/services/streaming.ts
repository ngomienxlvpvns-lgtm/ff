...
export class StreamingService {
	...
	private async finalizeStream(message: Message, tempFile: string | null): Promise<void> {
        ...
        if (tempFile) {
            // Chờ 3 giây để FFmpeg giải phóng hoàn toàn file trước khi xóa
            setTimeout(async () => {
                let retries = 3;
                while (retries > 0) {
                    try {
                        if (fs.existsSync(tempFile)) {
                            ...
                            logger.info(`Successfully deleted temp file: ${tempFile}`);
                        }
                        break; // Xóa thành công thì thoát vòng lặp
                    } catch (cleanupError) {
                        retries--;
                        if (retries === 0) {
                            logger.error(`Failed to delete temp file ${tempFile} after multiple retries:`, cleanupError);
                        } else {
                            // Chờ thêm 2 giây trước khi thử xóa lại
                            await new Promise(resolve => setTimeout(resolve, 2000));
                        }
                    }
                }
